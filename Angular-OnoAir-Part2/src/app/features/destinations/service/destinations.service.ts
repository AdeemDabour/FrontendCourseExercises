...
async updateDestination(id: string, updatedDestination: Destination): Promise<void> {
  ...
  if (docSnapshot.exists()) {
    await setDoc(destinationDoc, updatedDestination);
  } else {
    console.error(`Destination with ID ${id} does not exist.`);
  }
}
  ...
  async updateDestinationStatus(id: string, status: Status): Promise<void> {
    const destinationDoc = doc(this.firestore, this.collectionName, id).withConverter(destinationConverter);
    const destinationSnapshot = await getDoc(destinationDoc);
    const destinationData = destinationSnapshot.data();

    if (destinationData) {
      destinationData.status = status;
      await setDoc(destinationDoc, destinationData);
    }
  }
  ...
  getDestinationByNameOrCode(nameOrCode: string): Destination | undefined {
    return this.destinations.find(destination => destination.name === nameOrCode || destination.code === nameOrCode);
  }
