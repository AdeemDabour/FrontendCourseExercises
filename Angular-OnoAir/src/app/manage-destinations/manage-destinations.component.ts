...
export class ManageDestinationsComponent {
  dataSource = new MatTableDataSource<Destination>(DESTINATION_DATA);
  displayedColumns: string[] = ['city', 'airport', 'website', 'email', 'code'];

  actions = [
    {
      name: 'View Details',
      callback: (destination: Destination) => this.onViewDetails(destination),
    },
  ];

  onViewDetails(destination: Destination) {
    console.log('Viewing details for:', destination);
  }
}

const DESTINATION_DATA: Destination[] = [
  {
    city: "Tel Aviv",
